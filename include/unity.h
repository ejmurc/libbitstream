 ...
 * Parameterized Tests
 *     - you'll want to create a define of TEST_CASE(...), TEST_RANGE(...)
 and/or TEST_MATRIX(...) which basically evaluates to nothing
...
#define TEST_PRINTF(message, ...) \
  UnityPrintF(__LINE__, (message), ##__VA_ARGS__)
#endif

/* It is not necessary for you to call PASS. A PASS condition is assumed if
 * nothing fails. This method allows you to abort a test immediately with a PASS
 * state, ignoring the remainder of the test. */
...
/* Add source files to a test executable's compilation and linking. Ex:
 * TEST_SOURCE_FILE("sandwiches.c") */
#define TEST_SOURCE_FILE(a)

/* Customize #include search paths for a test executable's compilation. Ex:
 * TEST_INCLUDE_PATH("src/module_a/inc") */
...
#define TEST_ASSERT_BIT_HIGH(bit, actual)                                      \
  UNITY_TEST_ASSERT_BITS(((UNITY_UINT)1 << (bit)), (UNITY_UINT)(-1), (actual), \
                         __LINE__, NULL)
#define TEST_ASSERT_BIT_LOW(bit, actual)                                      \
  UNITY_TEST_ASSERT_BITS(((UNITY_UINT)1 << (bit)), (UNITY_UINT)(0), (actual), \
                         __LINE__, NULL)
...
#define TEST_ASSERT_FLOAT_NOT_WITHIN(delta, expected, actual)                 \
  UNITY_TEST_ASSERT_FLOAT_NOT_WITHIN((delta), (expected), (actual), __LINE__, \
                                     NULL)
...
#define TEST_ASSERT_FLOAT_ARRAY_WITHIN(delta, expected, actual, num_elements) \
  UNITY_TEST_ASSERT_FLOAT_ARRAY_WITHIN((delta), (expected), (actual),         \
                                       (num_elements), __LINE__, NULL)
...
#define TEST_ASSERT_GREATER_OR_EQUAL_FLOAT(threshold, actual)               \
  UNITY_TEST_ASSERT_GREATER_OR_EQUAL_FLOAT((threshold), (actual), __LINE__, \
                                           NULL)
...
#define TEST_ASSERT_DOUBLE_NOT_WITHIN(delta, expected, actual)                 \
  UNITY_TEST_ASSERT_DOUBLE_NOT_WITHIN((delta), (expected), (actual), __LINE__, \
                                      NULL)
...
#define TEST_ASSERT_DOUBLE_ARRAY_WITHIN(delta, expected, actual, num_elements) \
  UNITY_TEST_ASSERT_DOUBLE_ARRAY_WITHIN((delta), (expected), (actual),         \
                                        (num_elements), __LINE__, NULL)
...
#define TEST_ASSERT_GREATER_OR_EQUAL_DOUBLE(threshold, actual)               \
  UNITY_TEST_ASSERT_GREATER_OR_EQUAL_DOUBLE((threshold), (actual), __LINE__, \
                                            NULL)
...
#define TEST_ASSERT_FLOAT_ARRAY_WITHIN_MESSAGE(delta, expected, actual, \
                                               num_elements, message)   \
  UNITY_TEST_ASSERT_FLOAT_ARRAY_WITHIN((delta), (expected), (actual),   \
                                       (num_elements), __LINE__, (message))
...
#define TEST_ASSERT_GREATER_THAN_FLOAT_MESSAGE(threshold, actual, message) \
  UNITY_TEST_ASSERT_GREATER_THAN_FLOAT((threshold), (actual), __LINE__,    \
                                       (message))
...
#define TEST_ASSERT_LESS_OR_EQUAL_FLOAT_MESSAGE(threshold, actual, message) \
  UNITY_TEST_ASSERT_LESS_OR_EQUAL_FLOAT((threshold), (actual), __LINE__,    \
                                        (message))
...
#define TEST_ASSERT_DOUBLE_ARRAY_WITHIN_MESSAGE(delta, expected, actual, \
                                                num_elements, message)   \
  UNITY_TEST_ASSERT_DOUBLE_ARRAY_WITHIN((delta), (expected), (actual),   \
                                        (num_elements), __LINE__, (message))
...
#define TEST_ASSERT_GREATER_THAN_DOUBLE_MESSAGE(threshold, actual, message) \
  UNITY_TEST_ASSERT_GREATER_THAN_DOUBLE((threshold), (actual), __LINE__,    \
                                        (message))
#define TEST_ASSERT_GREATER_OR_EQUAL_DOUBLE_MESSAGE(threshold, actual,       \
                                                    message)                 \
  UNITY_TEST_ASSERT_GREATER_OR_EQUAL_DOUBLE((threshold), (actual), __LINE__, \
                                            (message))
...
#define TEST_ASSERT_LESS_OR_EQUAL_DOUBLE_MESSAGE(threshold, actual, message) \
  UNITY_TEST_ASSERT_LESS_OR_EQUAL_DOUBLE((threshold), (actual), __LINE__,    \
                                         (message))
