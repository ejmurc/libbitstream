...
static const char UNITY_PROGMEM UnityStrNullPointerForExpected[] =
    " Expected pointer to be NULL";
static const char UNITY_PROGMEM UnityStrNullPointerForActual[] =
...
static const char UNITY_PROGMEM UnityStrInvalidFloatTrait[] =
    "Invalid Float Trait";
#endif
...
const char UNITY_PROGMEM UnityStrErrDetailStack[] =
    "Unity Detail Stack Support Disabled";
...
static const char* UNITY_PROGMEM UnityStrDetailLabels[] =
    UNITY_DETAIL_LABEL_NAMES;
static const UNITY_COUNTER_TYPE UNITY_PROGMEM UnityStrDetailLabelsCount =
    sizeof(UnityStrDetailLabels) / sizeof(const char*);
static const char UNITY_PROGMEM UnityStrErrDetailStackEmpty[] =
    " Detail Stack Empty";
static const char UNITY_PROGMEM UnityStrErrDetailStackFull[] =
    " Detail Stack Full";
static const char UNITY_PROGMEM UnityStrErrDetailStackLabel[] =
    " Detail Label Outside Of UNITY_DETAIL_LABEL_NAMES: ";
static const char UNITY_PROGMEM UnityStrErrDetailStackPop[] =
    " Detail Pop With Unexpected Arguments";
#else
static const char UNITY_PROGMEM UnityStrDetail1Name[] = UNITY_DETAIL1_NAME " ";
static const char UNITY_PROGMEM UnityStrDetail2Name[] =
    " " UNITY_DETAIL2_NAME " ";
#endif
#endif
/*-----------------------------------------------
...
static void UnityAddMsgIfSpecified(const char* msg) {
...
#ifdef UNITY_DETAIL_STACK_SIZE
  {
    ...
    for (c = 0;
         (c < Unity.CurrentDetailStackSize) && (c < UNITY_DETAIL_STACK_SIZE);
         c++) {
      const char* label;
...
#define UNITY_FLOAT_OR_DOUBLE_WITHIN(delta, expected, actual, diff) \
  if (UNITY_IS_INF(expected) && UNITY_IS_INF(actual) &&             \
      (((expected) < 0) == ((actual) < 0)))                         \
    return 1;                                                       \
...
void UnityAssertWithinFloatArray(
  ...
  while (elements--) {
    ...
    if (!UnityFloatsWithin(in_delta + current_element_delta, *ptr_expected,
                           *ptr_actual)) {
      UnityTestResultsFailBegin(lineNumber);
      UnityPrint(UnityStrElement);
      UnityPrintNumberUnsigned(num_elements - elements - 1);
...
void UnityAssertGreaterOrLessFloat(const UNITY_FLOAT threshold,
  ...
  /* Checking for "not success" rather than failure to get the right result for
   * NaN */
  if (!(actual < threshold) && (compare & UNITY_SMALLER_THAN)) {
    failed = 1;
  }
...
void UnityAssertWithinDoubleArray(
  ...
  while (elements--) {
    ...
    if (!UnityDoublesWithin(in_delta + current_element_delta, *ptr_expected,
                            *ptr_actual)) {
      UnityTestResultsFailBegin(lineNumber);
      UnityPrint(UnityStrElement);
      UnityPrintNumberUnsigned(num_elements - elements - 1);
...
void UnityAssertGreaterOrLessDouble(const UNITY_DOUBLE threshold,
  ...
  if ((compare & UNITY_EQUAL_TO) &&
      UnityDoublesWithin(threshold * UNITY_DOUBLE_PRECISION, threshold,
                         actual)) {
    failed = 0;
  }
    ...
    switch (length) {
      case 1:
        ...
        if ((style & UNITY_DISPLAY_RANGE_INT) == UNITY_DISPLAY_RANGE_INT) {
          ...
          expect_val =
              (UNITY_INT) * (UNITY_PTR_ATTRIBUTE const UNITY_UINT8*)expected;
          actual_val =
              (UNITY_INT) * (UNITY_PTR_ATTRIBUTE const UNITY_UINT8*)actual;
      ...
      case 2:
        ...
        if ((style & UNITY_DISPLAY_RANGE_INT) == UNITY_DISPLAY_RANGE_INT) {
          ...
          expect_val =
              (UNITY_INT) * (UNITY_PTR_ATTRIBUTE const UNITY_UINT16*)expected;
          actual_val =
              (UNITY_INT) * (UNITY_PTR_ATTRIBUTE const UNITY_UINT16*)actual;
      ...
      case 8:
        ...
        if ((style & UNITY_DISPLAY_RANGE_INT) == UNITY_DISPLAY_RANGE_INT) {
          ...
          expect_val =
              (UNITY_INT) * (UNITY_PTR_ATTRIBUTE const UNITY_UINT64*)expected;
          actual_val =
              (UNITY_INT) * (UNITY_PTR_ATTRIBUTE const UNITY_UINT64*)actual;
      ...
      case 4:
        ...
        if ((style & UNITY_DISPLAY_RANGE_INT) == UNITY_DISPLAY_RANGE_INT) {
          ...
          expect_val =
              (UNITY_INT) * (UNITY_PTR_ATTRIBUTE const UNITY_UINT32*)expected;
          actual_val =
              (UNITY_INT) * (UNITY_PTR_ATTRIBUTE const UNITY_UINT32*)actual;
...
static enum UnityLengthModifier UnityLengthModifierGet(const char* pch,
                                                       int* length) {
    ...
    while (*pch) {
      ...
      if (*pch == '%') {
        ...
        if (pch != NULL) {
          int length_mod_size;
          enum UnityLengthModifier length_mod =
              UnityLengthModifierGet(pch, &length_mod_size);
          pch += length_mod_size;
          ...
          switch (*pch) {
            ...
            case 'u': {
              UNITY_UINT number;
              UNITY_EXTRACT_ARG(UNITY_UINT, number, length_mod, va,
                                unsigned int);
              UnityPrintNumberUnsigned(number);
              break;
            }
...
void UnityPopDetail(UNITY_DETAIL_LABEL_TYPE label,
  ...
  if ((Unity.CurrentDetailStackLabels[Unity.CurrentDetailStackSize - 1] !=
       label) ||
      (Unity.CurrentDetailStackValues[Unity.CurrentDetailStackSize - 1] !=
       value)) {
    UnityTestResultsFailBegin(line);
    ...
    if (argv[i][0] == '-') {
      switch (argv[i][1]) {
        ...
        case 'f': /* an alias for -n */
          UnityStrictMatch =
              (argv[i][1] == 'n'); /* strictly match this string if -n */
          if (argv[i][2] == '=') {
        ...
        case 'h':
          ...
          UNITY_PRINT_EOL();
          UnityPrint("-n NAME   Run only the test named NAME");
          UNITY_PRINT_EOL();
...
/*-----------------------------------------------*/
static int IsStringInBiggerString(const char* longstring,
  ...
  while (*lptr) {
    ...
    // If we didn't match and we're on strict matching, we already know we
    // failed
    if (UnityStrictMatch) {
      return 0;
    }
